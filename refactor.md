...
- [x] **Diverged duplicate regression-kind sets.** *(done: `REGRESSION_KINDS` frozenset in models.py:765)* `models.ChangeReport._compute_has_regression`
...
- [x] `classify.py`: `_ADDON_PRODUCTS` (139–144); local `import re as _re` (2882) shadowing
      the top-level import.
- [x] `pricing_tokens.py`: `OPERATOR_MARKERS` (117). Confirm and remove
      `_detect_decimal_separator` (129–166, no callers found).
- [x] `http.py`: `CachedSource` dataclass (91–98) + the never-read `cached` parameter
      threaded through `_request`/`get`; `HttpClient.head` (424–431, no callers).
- [x] `output.py`: `OutputPublisher.data_changed` (411–418) and `write_crawl_report`
      (420–422) — resolved via Phase 4: crawler now calls the publisher methods.
- [x] `discovery.py`: `_locale_for_country` (153–157, no callers; both branches return the
      same value); inline the trivial `_canonical_market_code` (179–185, ignores its arg).
- [x] `exceptions.py`: `FeePageStructureError` (107–110, never raised or imported).
- [x] `models.py` — unused `CrawlConfiguration` fields removed. Decision taken for
      `atomic`/`allow_partial`: kept parseable with deprecation warnings
      (`_warn_unused_atomic` / `_warn_unimplemented_allow_partial`).
- [x] `models.Source.detected_currency` (95): decision taken — **wired** through
      `http.py` / `extract.py` (detection result is now stored).
- [x] `normalize.py`: `normalize_country_code`, `normalize_locale`, `normalize_currency`
      now used by the `Market` validators (models.py imports them) — duplication removed,
      helpers kept.
...
- [x] `_classify_group` (1947–2209, 262 lines, F-rated): split into ~5 helpers along its
...
- [x] `_infer_conditions` (1402–1577, F): much of it is keyword → `FeeCondition(dimension,
...
- [x] `_infer_product_id` (1195–1290, F) *(done: unified `_PRODUCT_KEYWORDS` in classify/tables.py:236)*: the ~30 sequential `if _text_has(...)` branches
...
- [x] `_coverage_summary` (2768–2847): replace ~20 repeated
      `model_copy(update={X: X+1})` bumps with a status→field map + loop (~-30 lines).
- [x] Collapse the 6–8 ten-line keyword→value inferencers *(done: `_first_match` in classify/_util.py:35)* (`_infer_card_tier`,
...
- [x] `_validate_rule_calculator_ready` (565–667): each independent check block becomes its
...
✅ **Done** — both packages exist as planned (`classify/` with `tables/evidence/
dimensions/components/grouping/dedup/_util`, `validation/` with
`schemas/semantic_rules/publication`), public names re-exported.
...
- [x] **`DIRECT_LOCALE_MARKETS`** defined identically in `market_detection.py:12` and
      `discovery.py:145` → one definition.
- [x] **Market-share phrase detection** duplicated in `components.py:369–397` and
      `validation.py:745–763` (same list + regex) → one shared helper.
- [x] **Git-revision readers** *(done: `_git.py`)*: `crawler._crawler_revision` (59–90) vs
      `validation._crawler_submodule_revision` (116–131) → one helper.
- [x] **Staging-change detection**: `crawler._staging_changed`/`_files_equal` (425–463)
...
- [x] **Payment-method vocabulary**: centralized in `payment_methods.py` (used by
...
- [x] **crawler.crawl_market** (118–230) *(done: `_crawl_page` helper)*: extract the near-identical
      fetch-extract-append blocks for the pricing page and the LPM page into one helper.
- [x] **Duplicate small helpers** *(done: `_read_json`, `_condition_key`, `_condition_key_data` in `_common.py`)*: `regression._country_code` ==
...
**High impact — fix together (they compound)** — ✅ 1–4 done
